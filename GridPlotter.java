 ...
 *
 *  @author Na Ryoung Jessica Chang (based on a template provided by Alyce Brady)
 *  @version 10/8/2014
 **/
...
public class GridPlotter implements GridChangeListener
{
  ...
  // drawing methods
    ...
    {
      for(int row=0; row<theGrid.numCols() && row<theGrid.numRows(); row++)
      {
          placeColorBlock(row, row);
      }
    }
    public void onTriangleButtonClick()
    {
        for(int column=0; column<theGrid.numCols(); column++)
        {
            for(int row=column; row<theGrid.numRows(); row++)
            {
                placeColorBlock(row, column);
            }
        }
    ...
    {
        for(int column=theGrid.numCols()-1, row=0; row<theGrid.numRows() 
                && row<theGrid.numCols(); column--, row++)
        {
            placeColorBlock(row, column);
        }
    }
    ...
    public void onBorderButtonClick()
    {
        for(int row=0; row<theGrid.numRows(); row++)
        {
            for(int column=0; column<theGrid.numCols(); column++)
